...
btn1.addEventListener('click', ()=>{
    ...
    /*
        ...
        String 객체에서 정규 표현식 객체를 이용하는 메소드
            문자열.math(정규식) : 문자열에서 정규식 패턴의 값과 일치하는 값을 리턴한다
...
btn2.addEventListener('click', ()=> {
    ...
    area.innerHTML = '<h3>플래그문자</h3>';
    // replace() 메소드에서 '%&'는 패턴을 만족하는 문자열을 가리킨다.
    area.innerHTML += `/a/ : ${str.replace(/a/,'($&)')} <br>`;
    area.innerHTML += `/a/ : ${str.replace(/a/i,'($&)')} <br>`;
    area.innerHTML += `/a/ : ${str.replace(/a/g,'($&)')} <br>`;
    area.innerHTML += `/a/ : ${str.replace(/a/ig,'($&)')} <br>`;
    area.innerHTML += `/a/ : ${str.replace(/a/gi,'($&)')} <br>`;
});
...
btn3.addEventListener('click', ()=>{
    ...
    /*
        ...
        ?는 앞선 패턴이 최대 한번 이상 반복되는 문자열을 의미한다
    */
    area.innerHTML += '<h4>반복 검색</h4>';
    area.innerHTML += `/a{1,2}/g : ${str.replace(/a{1,2}/g, '($&)')}  <br>`;
    // {3,3}
    area.innerHTML += `/a{3}/g : ${str.replace(/a{3}/g, '($&)')}  <br>`;
    // 앞선 패턴이 최소 2번이상 반복되는 문자열 의미
    area.innerHTML += `/a{3}/g : ${str.replace(/a{2,}/g, '($&)')}  <br>`;
    // {1,}
    area.innerHTML += `/a{3}/g : ${str.replace(/a+/g, '($&)')}  <br>`;
    // {0,1}
    area.innerHTML += `/a{3}/g : ${str.replace(/a?/g, '($&)')}  <br>`;
    ...
    */
    ...
    str2 = 'Javascript Jquery Ajax';

    /*
        OR 검색
        [...] 내의 문자들 중 하나라도 존재할 경우
    */
